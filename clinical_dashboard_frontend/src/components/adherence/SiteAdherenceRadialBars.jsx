...
import { fetchSiteAdherenceDistribution } from '../../API';
    ...
    const fetchData = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const response = await fetchSiteAdherenceDistribution();
          setChartData(response.data);
        }
      } catch (err) {
        console.error("Error fetching adherence distribution data:", err);
        setError("Failed to load adherence distribution data");
